...
#my project partner can't access anydesk only signle user can access anydesk. so he also has to look into project
#ok come do opene the google meet link and share your screen , you will remain connected here?yes ok
...
def System():
    ...
    while (cap.isOpened()):
        ...
        if ret == True:
            ...
            # conditions
            if left_eye["no_glass_eye_close"] and right_eye["no_glass_eye_close"] >= 95.00:
                count += 1
                print(count)
            ...
            if emotion["happiness"] >= 97.0 and (happy or happy1):
                pygame.init()
                pygame.mixer.init()
                song = f'/Users/syedhusnain/PycharmProjects/Driver_alert/venv/music/happy.mp3'
                print("playing")
            ...
            if emotion["surprise"] >= 60.0 and (sad or sad1):
                pygame.init()
                pygame.mixer.init()
                song = f'/Users/syedhusnain/PycharmProjects/Driver_alert/venv/music/sad.mp3'
                print("playing")
                pygame.mixer.music.load(song)
                pygame.mixer.music.play()
                sad = False
...
def login():
    ...
    if __name__ == '__main__':
        ...
        try:
            ...
            # Pair 1
            image1 = 'venv/access_photo/DSC_0405.png'
            # Create a VideoCapture object and read from input file
            cap = cv2.VideoCapture(0)
            ...
            conn = sqlite3.connect('/Users/syedhusnain/PycharmProjects/Driver_alert/image_db.db')
            cursor = conn.cursor()
                    ...
                    if ret == True:
                        ...
                        if r == ("1"):
                            ...
                            if cv2.waitKey(25) & 0xFF == ord('q'):
                                ...
                                break
                            # Create a VideoCapture object and read from input file
                            cap = cv2.VideoCapture('venv/music/loading.mp4')
...
def browseFiles (text_entry,U):
    ...
    open_file = filedialog.askopenfilenames(
        filetypes=[("Image Files", ".png .jfif, .jpg, .jpeg")])  # returns a tuple with opened file's complete path
...
if __name__ == '__main__':
    ...
    width=screen_width-416
    height=screen_height-680
    root.geometry(f"{width}x768+{height}+120")
    
    # root.geometry("800x600+220+120")
    bg = PhotoImage(file="venv/back.png")

    background_label = Label(root, image=bg)
    background_label.place(x=0, y=0, relwidth=1, relheight=1)

    head = Label(root, text="Driver Alert System", font=("Phosphate", 50))
    head.place(x=275, y=30)

    pre_rec = Button(root, text="Login With Face", bg='#567', font=('Verdana', 18), fg='blue', command=login)
    pre_rec.configure(height=2, width=16)
    pre_rec.place(x=420, y=380)
    img = ImageTk.PhotoImage(Image.open("venv/icon.png"))
    l3 = Label(image=img, width=40, height=42)
    ...
    #
    l2 = Label(root, text="UserName ").place(x=330, y=221)
    ...
    l2 = Label(root, text="Password ").place(x=330, y=251)
    pws.place(x=420, y=250)

    ttk.Button(root, text="Login", command=lambda: login_with_pass(user, pws)).place(x=420, y=300)
    # # Add a Button to reveal the password
    ttk.Button(root, text="Show Password", command=show).place(x=660, y=250)

    # command = lambda: login(root)

    button_explore = Button(root,
                            text="add_user",width=26,
                            command=user_name).place(x=420, y=450)

    cam_button = Button(root,
                            text="add_user_by_cam",width=26,
                            command=user_name_for_cam).place(x=420, y=480)


    bd = Button(root,
                        text="create db",width=26,
                        command=create_db).place(x=420, y=510)

    exit = Button(root,text="X",background = "blue",foreground="black",command=exit).place(x=40,y=50)
